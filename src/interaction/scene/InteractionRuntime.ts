...
export class InteractionRuntime {
  ...
  constructor(options: InteractionRuntimeOptions) {
    ...
    this.glass = new GlassSystem(this.tuning, this.view.halfWidth, this.view.halfHeight)
    this.shuriken = new ShurikenSystem(this.tuning)
    this.fragments = new FragmentSystem(this.tuning)
    this.view.scene.add(this.glass.root, this.fragments.root, this.shuriken.root)

    this.pointer = new PointerInteraction({ camera: this.view.camera, surface: canvas })
      ...
      this.quietObserver = new IntersectionObserver(
        (entries) => {
          for (const entry of entries) {
            this.setQuiet(entry.isIntersecting && entry.intersectionRatio >= QUIET_THRESHOLD)
          }
        },
  ...
  private tick = (now: number) => {
    ...
    const delta = this.lastTime === 0 ? 1 / 60 : Math.min(MAX_STEP, (now - this.lastTime) / 1000)
    this.lastTime = now
    ...
    this.glass.setParallax(-progress * PARALLAX_RANGE * this.tuning.motionScale)
  ...
  private handlePick(): void {
    ...
    this.shuriken.throwAt(this.impactPoint, slotIndex, this.view.halfWidth, this.view.halfHeight)
  }
